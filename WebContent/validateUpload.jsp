...
String prodid = null;
			...
			while (itr.hasNext()) {
				...
				// else condition handles the submit button input
				 ...
				 if (!fi.isFormField()) {
					...
					if (!saveToDB) {
						out.print("<H3>Remote name: " + fi.getName() + " Local name: " + localName + "  Size: "
								+ fileSize + "</H3>");
						...
						String stmtSQL = "UPDATE product SET productImage = ? WHERE productId = ?;";
						PreparedStatement stmt = con.prepareStatement(stmtSQL);
						stmt.setBinaryStream(1, fi.getInputStream(), fileSize);
						stmt.setInt(2,Integer.parseInt(prodid));
						stmt.executeUpdate();

						out.println(
								"<H3>Inserted file: " + localName + " size: " + fileSize + " into database</H3>");
					}
				} else if(fi.isFormField() && fi.getFieldName().equals("this")){
					prodid = fi.getString();
	...
	<%!private String stripClientPath(String s) { // Strips the client path from a filename and returns only the filename itself
		if (s == null)
			return null;
